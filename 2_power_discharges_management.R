...
#! ---------------- Fase di carica per C2 -----------
#Riempiamo  il nuovo dataset
carica_C2 <- data.frame(Gruppo = unique(data$Gruppo), stringsAsFactors = FALSE)


#Gruppo
carica_C2$Gruppo <- unique(data$Gruppo)
...
for (group in seq(1, max(data$Gruppo) - 1)) {
  ...
  # Timestamp iniziale
  carica_C2$Timestamp_iniziale[group] <- as.character(subset_gruppo$Timestamp[1])
  
  #I iniziale finale e media
  carica_C2$I_finale[group] <- tail(subset_gruppo$HMI_IBatt_C2[subset_gruppo$HMI_IBatt_C2 > 0], 1)
  carica_C2$I_iniziale[group]<- head(subset_gruppo$HMI_IBatt_C2[subset_gruppo$HMI_IBatt_C2 > 0], 1)
  carica_C2$I_media[group] <- mean(subset_gruppo$HMI_IBatt_C2[subset_gruppo$HMI_IBatt_C2 > 0])
  
  #V iniziale finale e media
  carica_C2$V_finale[group] <- tail(subset_gruppo$HMI_VBatt_C2[subset_gruppo$HMI_IBatt_C2 > 0], 1)
  carica_C2$V_iniziale[group]<- head(subset_gruppo$HMI_VBatt_C2[subset_gruppo$HMI_IBatt_C2 > 0], 1)
  carica_C2$V_media[group] <- mean(subset_gruppo$HMI_VBatt_C2[subset_gruppo$HMI_IBatt_C2 > 0])
  
  # Durata scarica
  carica_C2$Durata_scarica[group] <- subset_gruppo$Durata_scarica[1]
  
  # Mese
  carica_C2$Mese[group] <- subset_gruppo$mese[1]
  
  # Stagione
  carica_C2$Stagione[group] <- subset_gruppo$stagione[1]
  
  # Wattora
  carica_C2$Wattora[group] <- subset_gruppo$Wattora_C2[1]
  
  # Amperora
  carica_C2$Amperora[group] <- subset_gruppo$Ah_C2[1]
    
}  

 saveRDS(carica_C2, "df_C2_C7/carica_C2.rds")
